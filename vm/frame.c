...
struct frame_table_entry
  {
    void *upage;               /* 유저 페이지 (가상 메모리) 주소 */
    void *kpage;               /* 물리 주소에 맵핑되는 커널 페이지 */

    struct hash_elem helem;    /* 해시 맵 */
    struct list_elem lelem;    /* 프레임 리스트 */
...
void
vm_frame_init ()//가상 메모리 initializing
{
...
vm_frame_allocate (enum palloc_flags flags, void *upage)
{
  ...
  if (frame_page == NULL) {//if frame table is full, 스왑이 필요하다
    ...
    swap_index_t swap_idx = vm_swap_out( f_evicted->kpage );//스왑 인덱스 설정한다.

    vm_supt_set_swap(f_evicted->t->supt, f_evicted->upage, swap_idx);//스왑을 설정한다.

    vm_supt_set_dirty(f_evicted->t->supt, f_evicted->upage, is_dirty);//더티 비트를 설정한다.

    vm_frame_do_free(f_evicted->kpage, true);// 스왑한 페이지를 free시켜준다.
  ...
  list_push_back (&frame_list, &frame->lelem);//list map에도 제일 뒤에 삽입한다.
 ...
 */
void
vm_frame_free (void *kpage)//가상 메모리 프레임을 free한다.
{
...
vm_frame_do_free (void *kpage, bool free_page)
{
  ...
  f = hash_entry(h, struct frame_table_entry, helem);//hash entry를 리턴받는다.

  hash_delete (&frame_map, &f->helem);//삭제한다.
  list_remove (&f->lelem);
  ...
  if (list_empty(&frame_list))
    ...
    for(clock_ptr=list_begin(&frame_list); clock_ptr!=list_end(&frame_list); clock_ptr=list_next(clock_ptr)){
      if(list_entry(clock_ptr, struct frame_table_entry, lelem)->history < oldest && (list_entry(clock_ptr, struct frame_table_entry, lelem)->pinned)==false){
        oldest= list_entry(clock_ptr, struct frame_table_entry, lelem)->history;//오래된 것을 찾으면 그 값과 포인터 저장(스왑될 수 있게)
        e=list_entry(clock_ptr, struct frame_table_entry, lelem);//포인터 저장
...
struct frame_table_entry* clock_frame_next(void)
{
  ...
  struct frame_table_entry *e = list_entry(clock_ptr, struct frame_table_entry, lelem);//그 가리키고 있는 요소를 반환함
  return e;
}

static void vm_frame_set_pinned(void *kpage,bool new_value){//pin을 true 또는 false로 설정하는 함수
  lock_acquire(&frame_lock);//mutual exclusion을 위한 뮤텍스

  struct frame_table_entry f_tmp;
  f_tmp.kpage = kpage;
  struct hash_elem *h = hash_find(&frame_map, &(f_tmp.helem));//커널 페이지를 해시맵에서 찾는다.
  ...
  f = hash_entry(h,struct frame_table_entry, helem);//엔트리를 반환받아서
...
void vm_frame_unpin(void *kpage){//프레임의 pin 해제
  vm_frame_set_pinned(kpage,false);
}

void vm_frame_pin(void *kpage){//프레임의 pin 설정
